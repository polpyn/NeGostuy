...
- to_bullet    — то же, что to_dash (синоним для совместимости): пункт с тире «— ...»
- to_dash      — пункт перечня с тире «— ...» (основной выбор для маркированных списков)
...
_SYSTEM_INSTRUCTION = (
    ...
    "Твоя задача — по смыслу и по соседним абзацам в окне понять, где заголовок, где связный текст, "
    "а где настоящий перечень однотипных пунктов; вернуть только JSON-действия без пересказа текста."
)
...
- to_dash       — маркированный пункт с длинным тире «— ...» (основной вид списка; не используй символ «•»).
- to_bullet     — синоним to_dash (устаревшее имя; оформление всё равно «— ...»).
...
- 2 = левый отступ 2.5 см — только для to_dash / to_bullet (пункты перечня под «N. …»).
...
2. ПОДПУНКТЫ С ПОЯСНЕНИЕМ. Абзацы вида "N. Prefix: пояснение..." ПОД заголовком секции —
   ...
   ИСКЛЮЧЕНИЕ — раздел «ХОД РАБОТЫ» (см. правило 9): там такие строки НЕ переводить в тире-список.

3. ВАЖНО про to_dash / to_bullet. Обычно — к абзацам с "N." или маркером "—/-/•".
   Для "Суть: ...", "Главная задача ...: ..." — всегда bold_prefix, НЕ to_dash: это поясняющий текст после заголовка «N. …»,
   ...
   to_dash с indent_level 2 под строкой «N. …». Не ставь to_dash длинным монолитным абзацам.

3а. ОДИН АБЗАЦ ПОСЛЕ «ГЛАВНАЯ ЗАДАЧА:» / «СУТЬ:». Смотри соседние [INDEX]: если предыдущий абзац — этикетка
   «Главная задача: …» или «Суть: …» (в т.ч. с кратким тезисом в той же строке), а текущий — одно связное пояснение
...
4. ПОДЗАГОЛОВКИ-ЭТИКЕТКИ. Абзац вида "Суть:", "Главная задача:", "Важное наблюдение:",
   ...
   Действие: bold_prefix, indent_level = 0 всегда (в т.ч. сразу под «N. Название секции»). На 2.5 см сдвигаются только to_dash/to_bullet.
...
8. ОБЫЧНЫЙ ТЕКСТ. Если абзац — обычный прозаический текст без нумерации, без двоеточия-подзаголовка
   ...
   Под абзацем-заголовком раздела «N. …» (например «1. Системы с разделением времени»): несколько коротких абзацев
   с требованиями, ограничениями или однотипными утверждениями об одном объекте
   («Пользователи не должны…», «Данные одного пользователя должны…») — если их ДВА и больше подряд и они параллельны по форме —
   оформляй to_dash, indent_level 2 (под «N. …»). Один такой абзац без «соседей»-тезисов — чаще связный текст, см. правило 3а; не насильно делай список.
...
9. ПРАКТИЧЕСКАЯ РАБОТА И ХОД РАБОТЫ.
   ...
   - После «Ход работы» до следующего крупного раздела строки «1. …», «2. …», «3. …» — последовательные шаги отчёта
     (файлы, модели, формы, представления, скриншоты). Для них ЗАПРЕЩЕНЫ to_dash и to_bullet: остаётся нумерация 1. 2. 3.
...
10. ДОКУМЕНТ БЕЗ НУМЕРАЦИИ. Даже без «1. 2. 3.» выделяй структуру по смыслу: названия блоков — bold_full;
   этикетки «Суть:», «Главная задача …:» — bold_prefix, indent_level 0; однородные короткие пункты-перечисления — to_dash (indent 2 под «N. …», иначе 1).
   Если в окне есть разделы «1. …», «2. …», подпункты из п.8 — to_dash с indent_level 2.
...
Ответ:
{
  "actions": [
    ...
    {"index": 38, "action": "to_dash",  "indent_level": 2, "reason": "подпункт под 'Файловый сервер'"},
    {"index": 39, "action": "to_dash",  "indent_level": 2},
    {"index": 40, "action": "to_dash",  "indent_level": 2},
    {"index": 41, "action": "bold_full",  "indent_level": 0, "reason": "заголовок секции"},
    {"index": 42, "action": "to_dash",  "indent_level": 2}
  ]
}
"""
...
_GOAL_OR_ESSENCE_LABEL = re.compile(
    r"(^|\s)(главная\s+задача|суть)\s*:",
    re.IGNORECASE,
)


def _skip_listify_after_goal_essence(
    doc: Document,
    idx: int,
    curr_text: str,
) -> bool:
    """
    После абзаца «Главная задача: …» / «Суть: …» один длинный поясняющий абзац
    (часто «Нужно…», «Не …») не должен становиться пунктом «—», даже если модель ошиблась.
    """
    if idx <= 0:
        return False
    prev = (doc.paragraphs[idx - 1].text or "").strip()
    if not prev or not _GOAL_OR_ESSENCE_LABEL.search(prev):
        return False
    t = (curr_text or "").strip()
    if len(t) < 35:
        return False
    low = t.lstrip().lower()
    if re.match(
        r"^(нужно|не\s+|следует|требуется|важно|чтобы|для\s+этого)\b",
        low,
    ):
        return True
...
def _heuristic_structure_fix(doc: Document) -> dict[str, Any]:
    ...
    for i, para in enumerate(paras):
        ...
        if nested_mode and is_top and num_last == nested_expected:
            action = _normalize_action({"index": i, "action": "to_dash", "indent_level": 2})
            if action:
